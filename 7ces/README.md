# 7CE's — Central Executive ("Seven Seas")
...
# 2. Create its .env, then add the 7CE's entries.
Copy-Item .env.example .env
#   Open .env and append the lines from 7ces/librechat/.env.example
...
Copy-Item "<repo>\7ces\librechat\librechat.yaml" .
Copy-Item "<repo>\7ces\librechat\docker-compose.override.yml" .
...
> Tip: `./7ces/start-7ces.ps1 -LibreChatPath C:\path\to\LibreChat` copies the two files in and runs
...
nothing is spent. Caps live in `7ces/gateway/.env`:
...
Tighten a single message with **Max Output Tokens**, or send a header `x-7ces-cap: 0.05`. The meter
...
- **Never commit a key.** Real keys live only in `7ces/gateway/.env` or a repo-root `.deepseek-key`
...
| Symptom | Fix |
|---|---|
| `/health` shows `deepseek:false` | add `DEEPSEEK_API_KEY` to `7ces/gateway/.env` (or a repo-root `.deepseek-key`) and restart the gateway |
...
- A literal per-message **dollar field** in the chat bar (LibreChat has none) — caps are enforced in
  the gateway; tighten per message via Max Output Tokens or the `x-7ces-cap` header.
