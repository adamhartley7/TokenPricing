<#
.SYNOPSIS
    Start 7CE's: the spend-guard gateway (and optionally bring up LibreChat).

.DESCRIPTION
    Run this from PowerShell:  ./start-7ces.ps1
    It will:
      1. create the gateway's Python venv (first run only) and install dependencies,
      2. ensure 7ces/gateway/.env exists (copied from .env.example if missing),
...
.EXAMPLE
    ./start-7ces.ps1
.EXAMPLE
    ./start-7ces.ps1 -LibreChatPath C:\Users\me\LibreChat
#>
param(
...
if (-not (Test-Path $EnvFile)) {
    ...
    Write-Host "Created 7ces/gateway/.env from the example. Open it and add your keys (or rely on" -ForegroundColor Yellow
...
if ($LibreChatPath -ne "") {
    ...
    Write-Host "Wiring 7CE's config into LibreChat at $LibreChatPath…" -ForegroundColor Cyan
    ...
    Write-Host "NOTE: add the keys from 7ces/librechat/.env.example to LibreChat's own .env first." -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "Next: start LibreChat (see 7ces/README.md). In short:" -ForegroundColor Cyan
    Write-Host "  1. Copy 7ces/librechat/librechat.yaml and docker-compose.override.yml into your LibreChat clone."
    Write-Host "  2. Add the entries from 7ces/librechat/.env.example to LibreChat's .env."
